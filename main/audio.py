...
def main():
    ...
    if os.path.isdir(AUDIO_PATH):
        # 目录模式 - 收集所有音频文件
        # print(f"\n扫描目录 {AUDIO_PATH} 中的音频文件...")
        supported_formats = ['.mp3']
        for ext in supported_formats:
            audio_files.extend(glob.glob(os.path.join(AUDIO_PATH, f'*{ext}')))
            audio_files.extend(glob.glob(os.path.join(AUDIO_PATH, f'*{ext.upper()}')))
        
        if not audio_files:
            print(f"❌ 在目录 {AUDIO_PATH} 中未找到支持的音频文件")
            return
        
        print(f" 找到 {len(audio_files)} 个音频文件")
    else:
        print(f"❌ 路径 {AUDIO_PATH} 既不是文件也不是目录")
        return
